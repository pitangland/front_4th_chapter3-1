...
describe('초기 상태', () => {
  ...
  it('currentDate는 오늘 날짜인 "2024-10-01"이어야 한다', () => {
    const { result } = renderHook(() => useCalendarView());
    assertDate(new Date('2024-10-01'), result.current.currentDate);
  });
...
it("주간 뷰에서 다음으로 navigate시 7일 후 '2024-10-08' 날짜로 지정이 된다", () => {
  ...
  assertDate(new Date('2024-10-08'), result.current.currentDate);
});
...
it("주간 뷰에서 이전으로 navigate시 7일 후 '2024-09-24' 날짜로 지정이 된다", () => {
  ...
  assertDate(new Date('2024-09-24'), result.current.currentDate);
});
...
it("월간 뷰에서 다음으로 navigate시 한 달 전 '2024-11-01' 날짜여야 한다", () => {
  ...
  assertDate(new Date('2024-11-01'), result.current.currentDate);
});
...
it("월간 뷰에서 이전으로 navigate시 한 달 전 '2024-09-01' 날짜여야 한다", () => {
  ...
  assertDate(new Date('2024-09-01'), result.current.currentDate);
});
