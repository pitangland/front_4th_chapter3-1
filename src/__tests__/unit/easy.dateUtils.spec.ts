...
describe('isDateInRange', () => {
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    ...
    // 범위에 포함되는 날짜와 포함되지 않는 날짜로 testDate 생성
    const testDate = [new Date('2024-03-18'), new Date('2025-03-18')];
...
describe('fillZero', () => {
  ...
  test("소수점이 있는 3.14를 5자리로 변환하면 '03.14'를 반환한다", () => {
    // padStart() 가 문자열로 동작하기 때문에 소수점을 포함한 길이를 고려하여 패딩
  ...
  test('value가 지정된 size보다 큰 자릿수를 가지면 원래 값을 그대로 반환한다', () => {
    expect(fillZero(135, 2)).toBe('135');
  });
});
...
describe('formatDate', () => {
  ...
  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    const date = new Date('2025-02-03');
    expect(formatDate(date, 18)).toBe('2025-02-18');
  });

  it('월이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    const date = new Date('2025-2-23');
    expect(formatDate(date)).toBe('2025-02-23');
  });

  it('일이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    const date = new Date('2025-02-3');
    expect(formatDate(date)).toBe('2025-02-03');
  });
});
